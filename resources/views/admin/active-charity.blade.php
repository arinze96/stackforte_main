                                                ...
                                                <table class="table">
                                                    ...
                                                    <tbody>
                                                        @foreach ($charities as $key => $charity)
                                                            <tr>
                                                                <td>{{ $key + 1 }}</td>
                                                                <td>{{ ucwords($charity->firstname) }}
                                                                    {{ ucwords($charity->lastname) }}</td>
                                                                <td>{{ ucwords($charity->currency) }}</td>
                                                                <td> {{ number_format($charity->amount, 0, '.', ',') }}
                                                                </td>
                                                                <td>{{ $charity->status == 0 ? 'unapproved' : 'Approved' }}
                                                                </td>

                                                                <td style="justify-content: space-between">
                                                                    <a
                                                                        href="{{ route('admin.charity.view', ['edit', $charity->id]) }}"><i
                                                                            class="icon-sm mdi mdi-pen text-warning ms-auto"
                                                                            style="margin-right: 5px"></i></a>
                                                                    <a class="delete_data"
                                                                        href="{{ route('admin.charity.view', ['delete', $charity->id]) }}"
                                                                        data-type="donation"><i
